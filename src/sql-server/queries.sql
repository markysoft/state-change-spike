...
BEGIN
    ...
    -- Only insert rows where the hash is different from the existing row
    -- (or where no existing row exists)
    INSERT INTO CollectStateLedger.dbo.CollectReturnStatus
    (
        SchoolName,
        LAEStab,
...
LEFT JOIN
    (
        -- Get ONLY the latest row per LAEStab + DCID using ROW_NUMBER()
        SELECT 
            LAEStab,
            DCID,
            Hash,
...
--             SELECT @DCID = DCID
--             FROM COLLECTPortal.dbo.DataCollection
--             WHERE DCBladeSQLDatabase = 'SchoolCensus2025_Spring'
...
--                CAST(dr.LowErrors AS VARCHAR), '|', 
--                CAST(dr.OKErrors AS VARCHAR))), 2) AS Hash,     
--                         DATEADD(DAY, -7, GETDATE()) AS UpdatedAt,                   
--                          @DCID as DCID
--             FROM COLLECTPortal.dbo.Organisation o
--             INNER JOIN COLLECTPortal.dbo.OrganisationRole orol
--                         ON orol.OrganisationID = o.OrganisationID
...
--             WHERE dr.DCID = @DCID
        ...
        --    DROP TABLE IF EXISTS CollectStateLedger.dbo.CollectReturnStatus
...
BEGIN
    ...
    DECLARE @DCID INT, @RowsAffected INT;
